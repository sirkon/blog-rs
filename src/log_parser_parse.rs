use super::*;
use crate::log_parser::{read_uvarint, read_uvarint_safe, LogParser};
use std::slice;
...
impl LogParser {
    ...
    pub(crate) unsafe fn parse_log_data<'a>(
        &mut self,
        src: &'a [u8],
    ) -> Result<&'a [u8], crate::log_parser::ErrorLogParse> { unsafe {
        if src.len() < 5 {
            return Err(crate::log_parser::ErrorLogParse::NoHeader);
        }

        let ptr = src.as_ptr() as *mut u8;
        if *ptr != 0xFF {
            return Err(crate::log_parser::ErrorLogParse::StartMarkerInvalid);
        }

        let record_crc = ptr.add(1).cast::<u32>().read_unaligned();
        let (length, size) = read_uvarint_safe(ptr.add(5), src.len() - 5);
        if size == usize::MAX {
            return Err(crate::log_parser::ErrorLogParse::RecordLengthInvalid);
        }
        if length as usize > self.max_log_size {
            return Err(crate::log_parser::ErrorLogParse::RecordLengthTooLarge);
        }
        if 5 + size + length as usize > src.len() {
            return Err(crate::log_parser::ErrorLogParse::RecordNeedMore);
        }
        let off = 5 + size;
        let record = slice::from_raw_parts(ptr.add(off), length as usize);
        let check = crc32c::crc32c(record);
        if check != record_crc {
            return Err(crate::log_parser::ErrorLogParse::RecordBroken);
        }
    ...
    pub(crate) unsafe fn parse_log_record<'a>(
        &mut self,
        src: &'a [u8],
    ) -> Result<(), crate::log_parser::ErrorLogParse> { unsafe {
        let ptr = src.as_ptr() as *mut u8;
        ...
        // TODO
        if version != 1 {
            return Err(crate::log_parser::ErrorLogParse::RecordVersionNotSupported(
                version,
            ));
        }
    ...
    pub(crate) unsafe fn parse_ctx<'a>(&mut self, src: &'a [u8]) { unsafe {
        self.ctx.reset();
        ...
        let mut cap = src.len();
        let mut parsing_state = crate::log_parser::CtxParsingState::Normal;
        let mut group_cap: usize = 0;
        let mut group_off: usize = 0;
        loop {
            match parsing_state {
                crate::log_parser::CtxParsingState::Normal => {
                    if off >= cap {
                ...
                crate::log_parser::CtxParsingState::Group => {
                ...
                crate::log_parser::CtxParsingState::Error => {
                    if off >= cap {
                ...
                crate::log_parser::CtxParsingState::ErrorEmbed => {
                    if off >= cap {
                        self.ctx.leave_group();
                        self.ctx.leave_group();
                        if self.caps.is_empty() {
            ...
            match kind {
                ...
                value_kind::ERROR => {
                    ...
                    parsing_state = crate::log_parser::CtxParsingState::Error;
                ...
                value_kind::GROUP => {
                    ...
                    parsing_state = crate::log_parser::CtxParsingState::Group;
                    self.ctx_size += 1;
                }
